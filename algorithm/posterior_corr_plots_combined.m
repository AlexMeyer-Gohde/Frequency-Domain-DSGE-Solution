...
Data = load('us.txt');
selection_vector=[4 2 3]; %data in column associated with which endo variable =[2]; would mean data in first column is associated with 2nd endo var
selection_scale=[1 4 4];
%data_corr = xcorr(selection_scale.*Data,horizon,'coeff');
%[T K]=size(Data);
%data_corr=data_corr(horizon+1:end-1,[K.*(0:K-1)]+[1:K]);

for j=1:horizon
data_corr(j,:)=diag(corr(Data(j:end,:),Data(1:end-j+1,:)))';
end
...
CC=[0	                0.447000000000000	0.741000000000000
    ...
    for k=1:length(variable_select)
        ...
        plot(tt, scale*squeeze(combined_bayesian_corr_50(variable_select(k),variable_select(k),1:horizon,ll)),'color', CC(ll,:), 'Linewidth', 1.5, 'HandleVisibility','off'); hold on
        plot(tt, o, 'k', 'Linewidth', 1.5, 'HandleVisibility','off');
        Hpatch  = [patch_time [scale*squeeze(combined_bayesian_corr_05(variable_select(k),variable_select(k),1:horizon,ll));flipdim(scale*squeeze(combined_bayesian_corr_95(variable_select(k),variable_select(k),1:horizon,ll)),1)]];
        patch(Hpatch(:,1),Hpatch(:,2),RGB_wide,'edgecolor',RGB_edge_wide, 'HandleVisibility','off'); hold on;

        Hpatch2  = [patch_time [scale*squeeze(combined_bayesian_corr_16(variable_select(k),variable_select(k),1:horizon,ll));flipdim(scale*squeeze(combined_bayesian_corr_84(variable_select(k),variable_select(k),1:horizon,ll)),1)]];
        patch(Hpatch2(:,1),Hpatch2(:,2),RGB_middle,'edgecolor',RGB_edge_middle, 'HandleVisibility','off');
        end
        %Median and zero line:
        for ll=1:4
        plot(tt, scale*squeeze(combined_bayesian_corr_50(variable_select(k),variable_select(k),1:horizon,ll)), 'color', CC(ll,:), 'Linewidth', 2);
        end
        plot(tt, o, 'k', 'Linewidth', 1.5, 'HandleVisibility','off');
        data_location=selection_vector==variable_select(k);
        if sum(data_location)>0
            plot(tt, scale*data_corr(:,data_location), 'k', 'Linewidth', 2);
            legend('AR (1)', 'MA(1)', 'Log Lag', 'Log Harmonic Lag','Data')
        else
        ...
        eval(sprintf('title(''Autocorrelation of %s '')',ENDOGENOUS_VARIABLE_NAMES{variable_select(k)}))
        grid on;
