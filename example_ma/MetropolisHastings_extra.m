% Adds to a sample from the posterior using Metropolis Haastings
%
% ALGORITHMS
...
load MH_candidate
load mhdraws
Thetasim_old=Thetasim;
logposterior_old=logposterior;
Nsim          = input('How Many more Posterior Draws?:  ');
disp('                                                                  ');
disp('                                                                  ');
...
c             = 0.25;
c0            = 0.25;
Nburn         = 0;%int32(0.50*Nsim)+2;
Thetasim      = zeros(Nsim,13);%zeros(Nsim,15);

% Initialize by taking a draw from a distribution centered at mode
go_on = 1;
while go_on == 0
   Thetac = mvnrnd(mode',c0*Sigma);
    go_on = (Thetac(8)>=0)*(Thetac(9)>=-1)*(Thetac(10)>=-1)*(Thetac(8)<=1)*(Thetac(9)<=1)*(Thetac(10)<=1)*(Thetac(2)<=1);  % bounds
end
Thetasim(1,:) = Thetac;%Thetasim(1,:) = [Thetac rand rand];
...
for i=1:Nsim
    ...
    Thetac = mvnrnd(Thetasim(i,1:13),c*Sigma);
    CheckBounds = (Thetac(8)>=0)*(Thetac(9)>=-1)*(Thetac(10)>=-1)*(Thetac(8)<=1)*(Thetac(9)<=1)*(Thetac(10)<=1)*(Thetac(2)<=1);  % bounds
%Thetac= [Thetac rand rand];
    if CheckBounds == 1 
       ...
       if objc == -Inf
          ...
          Thetasim(i+1,1:13) = Thetasim(i,1:13);
...
Thetasim    = [Thetasim_old;Thetasim];

logposterior= [logposterior_old;logposterior];

save Matfiles/mhdraws_extra Thetasim logposterior   % Save posterior draws

[Nsim,Npam] = size(Thetasim(:,1:13));
...
rmean(1,:)=Thetasim(1,1:13);
for i=2:Nsim
    rmean(i,:) =((i-1)*rmean(i-1,:)+Thetasim(i,1:13))/i;
end
